...
CI ジョブはコミット済みの `package-lock.json` から npm 依存関係をインストールします。

- `npm run ci:min` はロック済みの依存関係グラフをインストールします。
  ライフサイクルスクリプトは実行されず、`package.json` とロックファイルが同期していない場合、インストールは失敗します。
- サイトをビルドするジョブは、インストール後に `npm run ci:prepare` を実行します。
  これは `hugo-extended`（唯一再有効化された依存関係フック）をリビルドし、リポジトリ独自の `prepare` セットアップを実行します。
- devcontainer はかわりに `npm run install:safe` を使用します。
  契約は同じですが、`netlify-cli` などのローカルツールを含むオプション依存関係を保持します。
...
- **`/fix:refcache`**（非推奨）は `fix:refcache` 互換エイリアスを経由して引き続き実行されます。
  結果コメントは `/fix:link-cache` を案内します。
