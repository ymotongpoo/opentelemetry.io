...
CI ジョブは、サイト全体の[インストール契約](/site/build/dependencies/#install-contracts)に従って npm の依存関係をインストールします。
ロックファイルに完全一致し、スクリプトを実行しない方式で、ビルドジョブではレビュー済みの Hugo リビルドのみを再有効化します。
