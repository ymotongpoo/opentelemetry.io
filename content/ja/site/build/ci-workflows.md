...
CI ジョブはサイト全体の[インストールの動作仕様](/site/build/dependencies/#install-contracts)に従って npm の依存関係をインストールします。
ロックファイルに厳密に従い、スクリプトを実行しない方式で、ビルドジョブのみがレビュー済みの Hugo リビルドを再有効化します。
